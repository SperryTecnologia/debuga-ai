...
export default function AccountPage() {
  ...
  // Phone edit state (contact only, no OTP)
  ...
  // phoneVerified removed — phone is contact-only, no verification required
  ...
  const updatePhoneMutation = trpc.account.updatePhone.useMutation({
    onSuccess: () => {
  ...
  async function handleSavePhone() {
    ...
    setSavingPhone(true);
    try {
      await updatePhoneMutation.mutateAsync({ phone: cleaned });
    } finally {
        ...
        <motion.div initial="hidden" animate="visible" variants={stagger}>
          {profileLoading ? (
            ...
            <>
                      ...
                      <div className="flex items-center gap-2">
                        {editingPhone ? (
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <Button size="sm" variant="ghost" className="h-7 px-2" onClick={handleSavePhone} disabled={savingPhone}>
                              {savingPhone ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle className="w-3.5 h-3.5 text-green-400" />}
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => setEditingPhone(false)}>
                              <span className="text-xs text-muted-foreground">✕</span>
                            </Button>
                          </div>
                        ) : (
                          <button
                            onClick={() => { setPhoneNumber(profile?.phone || ""); setEditingPhone(true); }}
              ...
              {/* Phone section removed - inline editing in profile card above */}
