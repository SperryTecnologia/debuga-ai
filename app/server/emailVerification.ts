...
async function sendVerificationEmail(
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
      <h2 style="color: #1a1a1a; margin-bottom: 8px;">${fromName}</h2>
      <p style="color: #555; font-size: 15px; line-height: 1.5;">
        Seu código de verificação é:
      </p>
      <div style="background: #f4f4f5; border-radius: 8px; padding: 24px; text-align: center; margin: 24px 0;">
        <span style="font-size: 32px; font-weight: 700; letter-spacing: 4px; color: #18181b;">${code}</span>
      </div>
      <p style="color: #555; font-size: 14px; line-height: 1.5;">
        Este código expira em ${TOKEN_EXPIRY_MINUTES} minutos.<br/>
        Se você não solicitou esta verificação, ignore este e-mail.
      </p>
      ${verifyUrl ? `<p style="margin-top: 16px;"><a href="${verifyUrl}" style="background: #22c55e; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: 500;">Verificar E-mail</a></p>` : ""}
      <hr style="border: none; border-top: 1px solid #e5e5e5; margin: 32px 0 16px;" />
      <p style="color: #999; font-size: 12px;">
        ${fromName} — Este é um e-mail automático, não responda.
      </p>
    </div>
  `;
...
async function sendPasswordResetEmail(
  ...
  const subject = `[${fromName}] Redefinição de senha: ${code}`;
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
      <h2 style="color: #1a1a1a; margin-bottom: 8px;">${fromName}</h2>
      <p style="color: #555; font-size: 15px; line-height: 1.5;">
  ...
  app.post("/api/auth/resend-verification", async (req: Request, res: Response) => {
    try {
      ...
      if (!canResend(cooldownKey)) {
        res.status(429).json({ error: "Aguarde 1 minuto antes de solicitar novo código." });
        return;
      }
  ...
  app.post("/api/auth/forgot-password", async (req: Request, res: Response) => {
    try {
      ...
      if (!user || user.authProvider !== "local") {
        res.json({ success: true, message: "Se o e-mail estiver cadastrado, você receberá um código." });
        return;
      }
      ...
      await sendPasswordResetEmail(normalizedEmail, result.code, appName);
      markResent(cooldownKey);
