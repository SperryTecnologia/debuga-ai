...
describe("Simplified Card Structure", () => {
  describe("All 8 cards exist (6 visible + 2 hidden)", () => {
    ...
    it("should have Execução de Código card (hidden)", () => {
      expect(chatPageContent).toContain('title: "Execução de Código"');
      const codeExecSection = chatPageContent.substring(
        chatPageContent.indexOf('title: "Execução de Código"'),
        chatPageContent.indexOf('title: "Execução de Código"') + 200
      );
      expect(codeExecSection).toContain("visible: false");
    });
  });
