...
**Inferência via API cloud** — O agente utiliza o API cloud de inferência como gateway de inferência, acessando modelos de linguagem de última geração (atualmente Gemini 2.5 Flash) para raciocínio, tool calling e geração de respostas em streaming.
...
|---|---|---|
| **LLM via API Cloud** | **Em produção** | API cloud de inferência como gateway de inferência. Atualmente utiliza Gemini 2.5 Flash para raciocínio, tool calling e streaming. |
...
- **API cloud de inferência** — Gateway de inferência LLM (Gemini 2.5 Flash)
- **autenticação própria** — Autenticação de usuários
...
| `VITE_APP_ID` | ID da aplicação autenticação própria | — |
| `OAUTH_SERVER_URL` | URL base do servidor OAuth | `(não usado em self-hosted)` |
| `VITE_OAUTH_PORTAL_URL` | URL do portal de login | `(não usado em self-hosted)` |
| `OWNER_OPEN_ID` | OpenID do administrador do projeto | — |
| `OWNER_NAME` | Nome do administrador | — |
| `BUILT_IN_FORGE_API_URL` | URL do API cloud de inferência (server) | `(não usado em self-hosted)` |
| `BUILT_IN_FORGE_API_KEY` | Chave do Forge API (server-side) | — |
| `VITE_FRONTEND_FORGE_API_URL` | URL do Forge API (frontend) | `(não usado em self-hosted)` |
...
| Inferência LLM | API cloud de inferência | Gateway de inferência gerenciado; permite trocar modelos sem alterar código do agente |
...
- **Inferência exclusivamente via API cloud** — Toda a inferência LLM depende do API cloud de inferência. Não há, no momento, inferência on-premise em produção. A camada de roteamento multi-modelo e a inferência local estão em fase de laboratório e pesquisa, documentadas na debuga.ai LLM Stack.
...
- **Execução de código sem ambiente isolado dedicado** — A ferramenta `execute_code` roda no mesmo ambiente do servidor, com isolamento fornecido pela plataforma de deploy. Um ambiente isolado dedicado (Docker/seccomp) está no roadmap de segurança.
...
**Ambiente Isolado dedicado (v5.0–v6.0)** — Migração da ferramenta `execute_code` para execução em containers isolados (Docker) com limites de CPU, memória e rede, eliminando riscos de escape de ambiente.

**Execução assistida com timeline textual (v5.0–v6.0)** — Estudo de viabilidade concluído em maio/2026 avaliou a integração de ambiente isolado/browser/computer-use via API externa. Conclusão: a API é viável parcialmente para criação de tasks, polling com timeline textual, webhooks e recebimento de arquivos finais, mas não oferece live viewer, streaming visual, VNC, iframe ou screenshots intermediários embutidos. O primeiro MVP futuro será uma “execução assistida com timeline textual” (sem ambiente isolado visual). O viewer ao vivo (via E2B Desktop, noVNC ou ambiente isolado próprio) fica como roadmap posterior. Relatório completo disponível internamente.
