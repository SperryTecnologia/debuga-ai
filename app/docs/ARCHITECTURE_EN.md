...
All LLM inference in the current production deployment is handled through the **API cloud de inferência** — a managed inference gateway. The system sends structured prompts (system instructions + conversation history + tool definitions) and receives streamed completions.

| Component | Status | Description |
|---|---|---|
| **API cloud de inferência** | **In production** | Managed gateway providing access to state-of-the-art models (currently Gemini 2.5 Flash) |
...
| API cloud de inferência | Cloud Provider | **Production** | Managed inference gateway; currently serves Gemini 2.5 Flash |
...
**Code Execution:** The `execute_code` tool runs user-provided code in `/tmp` with a 30-second timeout and 50KB output limit. The deployment platform provides additional process-level isolation. A dedicated isolated environment (Docker-based) is planned for future versions to provide stronger isolation guarantees.
...
| **API cloud de inferência** | LLM inference (Gemini 2.5 Flash) | `server/_core/llm.ts` |
| **OAuth legado** | User authentication | `server/_core/oauth.ts` |
| **S3-compatible storage** | File and artifact storage | `server/storage.ts` |
| **Serviço de geração de imagens** | AI image generation | `server/_core/imageGeneration.ts` |
...
[Cloudflare CDN] → [debuga.ai / www.debuga.ai]
                  ...
                  [Cloud Platform]
                    ...
                    ├── TiDB Database
                    ├── S3 Storage
                    └── API cloud de inferência (LLM Inference)
```
