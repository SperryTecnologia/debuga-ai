...
let _dbConnectionTested = false;
let _dbConnectionError: string | null = null;
    ...
    try {
      ...
      if (!_dbConnectionTested) {
        try {
          ...
          _dbConnectionError = testErr?.message || String(testErr);
          console.error(`[Database] Connection test FAILED: ${_dbConnectionError}`);
          console.error(`[Database]   DATABASE_URL host: ${new URL(process.env.DATABASE_URL).hostname}`);
          console.error(`[Database]   DATABASE_URL user: ${new URL(process.env.DATABASE_URL).username}`);
          console.error(`[Database]   Likely cause: password mismatch. Run: bash scripts/sync-postgres-password.sh`);
          // Don't null out _db — let individual queries fail with clear errors
          // This prevents infinite retry of pool creation
        }
      }
    } catch (error) {
...
export function getDbStatus() {
  return {
    poolCreated: !!_db,
    connectionTested: _dbConnectionTested,
    connectionError: _dbConnectionError,
...
export async function testDbConnection(): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    if (!db) return { ok: false, error: "No database pool" };
    // Run a real query
...
const ESSENTIAL_TABLES = ["users", "conversations", "messages", "credits", "subscriptions", "app_settings", "ai_instructions", "knowledge_base", "ai_provider_logs"];

export async function checkSchema(): Promise<{ ready: boolean; missingTables: string[] }> {
  try {
    const db = await getDb();
    if (!db) return { ready: false, missingTables: ESSENTIAL_TABLES };
    const result = await db.execute(sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'`);
    const existingTables = (result.rows || []).map((r: any) => r.table_name);
    const missingTables = ESSENTIAL_TABLES.filter(t => !existingTables.includes(t));
    return { ready: missingTables.length === 0, missingTables };
  } catch (err: any) {
    console.error("[Database] checkSchema error:", err.message);
    return { ready: false, missingTables: ESSENTIAL_TABLES };
  }
}
...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  const db = await getDb();
  if (!db) {
    throw new Error("Database connection unavailable — cannot upsert user");
  }
  ...
  try {
    ...
    // Handle emailVerified (e.g., Google OAuth users are always verified)
...
export async function getUserByOpenId(openId: string) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database connection unavailable — cannot get user");
  }
...
export async function getUserByEmail(email: string) {
   const db = await getDb();
  if (!db) throw new Error("Database connection unavailable — cannot get user by email");
  const rows = await db.select().from(users).where(eq(users.email, email)).limit(1);
  return rows[0] || undefined;
}
export async function getUserById(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database connection unavailable — cannot get user by id");
