...
export const passwordResetLimiter = createRateLimiter({
  ...
  windowMs: 15 * 60 * 1000,
  maxRequests: 5,
  message: "Muitas solicitações de redefinição. Tente novamente em alguns minutos.",
});
...
/**
 * Verification code resend: 5 per 5 minutes per user (not per IP)
 * Uses session cookie to identify user; falls back to IP only if no session
 */
export const verificationLimiter = createRateLimiter({
  ...
  keyGenerator: (req) => {
    // Try to use session cookie for per-user limiting instead of per-IP
    const cookies = req.headers.cookie || "";
    const sessionMatch = cookies.match(/app_session_id=([^;]+)/);
    if (sessionMatch) return `session_${sessionMatch[1].substring(0, 32)}`;
