...
type TurnstileVerifyResult = { success: true } | { success: false; reason: "timeout-or-duplicate" | "invalid" | "network_error" };
...
async function verifyTurnstile(token: string, remoteIp: string): Promise<TurnstileVerifyResult> {
  ...
  if (!secret) {
    console.log("[Turnstile] No secret key configured — skipping server verification");
    return { success: true };
  }

  console.log(`[Turnstile] Validating with Cloudflare: token=${token.slice(0, 12)}..., ip=${remoteIp}`);
  ...
  try {
    ...
    if (data.success) {
      console.log(`[Turnstile] Cloudflare OK: hostname=${data.hostname}, ts=${data.challenge_ts}`);
      return { success: true };
    }

    const errorCodes = data["error-codes"] || [];
    console.error(`[Turnstile] Cloudflare REJECTED:`, {
    ...
    // Detect timeout-or-duplicate specifically (token was already used or expired)
    if (errorCodes.includes("timeout-or-duplicate")) {
      return { success: false, reason: "timeout-or-duplicate" };
    }
...
export function registerLocalAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/local/register", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`[Turnstile] Register check: enabled=${ENV.enableTurnstile}, secretKey=${!!ENV.turnstileSecretKey}, siteKey=${!!ENV.turnstileSiteKey}, tokenReceived=${!!turnstileToken}, tokenLength=${turnstileToken?.length || 0}`);

      if (ENV.turnstileSecretKey && ENV.enableTurnstile) {
        if (turnstileToken) {
          // Token provided — verify with Cloudflare
          const captchaResult = await verifyTurnstile(turnstileToken, req.ip || "");
          console.log(`[Turnstile] Verification result: ${captchaResult.success ? "PASS" : "FAIL (" + (captchaResult as any).reason + ")"}, ip=${req.ip}`);
          if (!captchaResult.success) {
            const reason = (captchaResult as { reason: string }).reason;
            if (reason === "timeout-or-duplicate") {
              res.status(400).json({ error: "A verificação expirou. Clique novamente para validar.", detail: "turnstile_expired" });
            } else {
              res.status(400).json({ error: "Falha na verificação CAPTCHA. Tente novamente.", detail: "turnstile_failed" });
            }
            return;
          }
        } else if (ENV.turnstileSiteKey) {
          // Site key is configured (widget should have rendered) but no token sent
          // This likely means VITE_TURNSTILE_SITE_KEY was not embedded in the frontend build
          console.warn(`[Turnstile] BLOCKED: Backend has siteKey but frontend sent no token. Likely VITE_TURNSTILE_SITE_KEY missing from Docker build args.`);
          res.status(400).json({
            error: "Verificação de segurança não carregou. Recarregue a página e tente novamente.",
            detail: "turnstile_widget_not_loaded",
          });
          return;
        } else {
          // No site key configured — widget couldn't render, skip verification
          console.log(`[Turnstile] Skipped: no siteKey configured, widget cannot render`);
        }
      } else {
        console.log(`[Turnstile] Disabled or no secretKey — skipping verification`);
      }
      ...
      if (err.message?.includes("Database connection unavailable")) {
        res.status(503).json({ error: "Serviço temporário indisponível. Tente novamente em instantes." });
        return;
      }
      if (err.message?.includes("relation") && err.message?.includes("does not exist")) {
        console.error("[LocalAuth] SCHEMA NOT MIGRATED — run: bash scripts/migrate-database.sh --env .env");
        res.status(503).json({ error: "Sistema em manutenção. Banco de dados sendo configurado. Tente novamente em alguns minutos." });
        return;
      }
  ...
  app.post("/api/auth/local/login", async (req: Request, res: Response) => {
    try {
      ...
      // Verify Turnstile CAPTCHA on login (if configured and enabled for login)
      console.log(`[Turnstile] Login check: enabled=${ENV.enableTurnstile}, onLogin=${ENV.turnstileOnLogin}, secretKey=${!!ENV.turnstileSecretKey}, tokenReceived=${!!turnstileToken}`);
      if (ENV.turnstileSecretKey && ENV.turnstileOnLogin && ENV.enableTurnstile) {
        if (turnstileToken) {
          const captchaResult = await verifyTurnstile(turnstileToken, req.ip || "");
          console.log(`[Turnstile] Login verification: ${captchaResult.success ? "PASS" : "FAIL (" + (captchaResult as any).reason + ")"}`);
          if (!captchaResult.success) {
            const reason = (captchaResult as { reason: string }).reason;
            if (reason === "timeout-or-duplicate") {
              res.status(400).json({ error: "A verificação expirou. Clique novamente para validar.", detail: "turnstile_expired" });
          ...
          console.warn(`[Turnstile] Login BLOCKED: siteKey set but no token from frontend`);
          res.status(400).json({
            error: "Verificação de segurança não carregou. Recarregue a página.",
            detail: "turnstile_widget_not_loaded",
          });
          return;
        }
  ...
  // ─────────────────────────────────────────────────────────────────
  // POST /api/auth/local/logout — Clear session cookiee
  // ─────────────────────────────────────────────────────────────────
