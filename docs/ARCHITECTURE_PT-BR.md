...
Toda a inferência LLM no deploy de produção atual é gerenciada através da **Manus Forge API** — um gateway de inferência gerenciado. O sistema envia prompts estruturados (instruções de sistema + histórico de conversa + definições de ferramentas) e recebe completions em streaming.

| Componente | Status | Descrição |
|---|---|---|
| **Manus Forge API** | **Em produção** | Gateway gerenciado com acesso a modelos de última geração (atualmente Gemini 2.5 Flash) |
...
| Manus Forge API | Provedor Nuvem | **Produção** | Gateway de inferência gerenciado; atualmente serve Gemini 2.5 Flash |
...
| **Manus Forge API** | Inferência LLM (Gemini 2.5 Flash) | `server/_core/llm.ts` |
...
[Cloudflare CDN] → [debuga.ai / www.debuga.ai]
                  ...
                  [Plataforma Cloud]
                    ...
                    ├── Banco de Dados TiDB
                    ├── Storage S3
                    └── Manus Forge API (Inferência LLM)
```
